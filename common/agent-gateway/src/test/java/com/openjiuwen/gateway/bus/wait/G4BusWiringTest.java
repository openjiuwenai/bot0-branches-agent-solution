...
/**
 * Unit tests for {@link G4BusWiring}.
 *
 * @since 2026-07-24
 */
class G4BusWiringTest {
    ...
    @Test
    void rejectedComplete() {
        register();
        wiring.onFold(InvocationResponseStatus.REJECTED, T, M, "rej");
        assertThat(g4.isCompleted(T, M)).contains(true);
    }
    ...
    void failedComplete() {
        register();
        wiring.onFold(InvocationResponseStatus.FAILED, T, M, "fail");
        assertThat(g4.isCompleted(T, M)).contains(true);
    }

    @Test
    void acceptedTimeoutComplete() {
        register();
        wiring.onFold(InvocationResponseStatus.ACCEPTED_WITH_TASK, T, M, "accepted");
        assertThat(g4.isCompleted(T, M)).contains(true);
    }
    ...
    void produceFailAbort() {
        register();
        wiring.onAbort(T, M);
        assertThat(g4.isCompleted(T, M)).isEmpty();
    }

    @Test
    void acceptTimeoutAbort() {
        register();
        wiring.onFold(InvocationResponseStatus.UNKNOWN, T, M, null);
        assertThat(g4.isCompleted(T, M)).isEmpty();
    }

    @Test
    void replayAfterCompleteNoReEnqueue() {
        register();
        wiring.onFold(InvocationResponseStatus.COMPLETED_RESPONSE, T, M, "result");
        var d = g4.check(T, M, FP);
        assertThat(d.outcome()).isEqualTo(IdempotencyRule.Outcome.REPLAY);
    }

    @Test
    void retryAfterAbortReRegisters() {
        register();
        wiring.onAbort(T, M);
        var d = g4.check(T, M, FP);
