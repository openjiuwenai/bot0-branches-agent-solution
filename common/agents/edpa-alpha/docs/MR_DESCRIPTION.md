...
EDPA-alpha（与 pev 平级，common/agents/ 下，互不依赖）
  ...
  ├── rail: DataFlowObserverRail（OpenTelemetry trace span，FULL/SIMPLE/OFF 配置降档）
...
- **DataFlow 可观测**（DataFlowObserverRail）：OpenTelemetry trace span 架构（一开始设计 span，不先简单后补）。三级配置降档 FULL（OTel span export）/ SIMPLE（record list）/ OFF（零开销）。泛化 payload 读取（兼容 PEVAgent extra payload + ReActAgent ModelCallInputs/ToolCallInputs）。
...
edpa-alpha → react-rails（ReplanRail / CriteriaReplanBridgeRail / RootCauseRail / RailInvocationState / CriteriaVerifier）
edpa-alpha → opentelemetry-api 1.55.0 + opentelemetry-sdk-trace 1.55.0（DataFlowObserverRail trace span）
```
