...
class VersatileInterruptRail(BaseInterruptRail):
    ...
    @staticmethod
    def _has_lite_todolist(ctx) -> bool:
        try:
            todos = ctx.session.get_state("lite_todolist") or []
        except Exception:
            return False
    ...
    def _create_intercepted_tool_span(ctx, tool_name: str, tool_args: dict, result: Any = None) -> None:
        ...
        with tracer.start_as_current_span(
            ...
            # 被拦截工具场景：现有 Rail 不打 TAG 日志，此处单独打
            to_logger(
                level=Level.INFO,
                message=metric_data,
    ...
    async def resolve_interrupt(
        ...
        if cascade_result is not None:
            ...
            result = await self._handle_cascade_resume(ctx, tool_name, tool_args, cascade_result)
    ...
    async def _handle_cascade_resume(self, ctx, tool_name: str, tool_args: dict, cascade_result):
        ...
        # 注入持久化字段到 SKILL_INPUT，使归一化脚本能读取并透传
        skill_input["history_info"] = ctx.session.get_state("history_info") or []
        
        # ═══════════════════════════════════════════════════════════════════════════
        # input_key 数据注入：从 tool_data_channel 读取前序工作流结果
        # ═══════════════════════════════════════════════════════════════════════════
        input_key = tool_args.get("input_key", "")
        if input_key:
            ...
            if input_data:
                ...
                logger.warning(
                    f"[VersatileInterruptRail] input_key 未命中："
        ...
        # rail 负责解析 key → 话术文本：
        #   - interrupt_start 事件：走 response_template 机制，由 agent_stream() 末尾直接 yield InterruptStartEvent
        #   - tool_end / todo_end 事件：走 write_stream 机制，通过 _drain_ui_notices() 在 tool_end 边界输出
        # 同时从 tool_result 中 pop 掉，避免 LLM 看到内部字段。
                ...
                if notice_event and notice_text:
                    if notice_event == "interrupt_start":
                        ...
                        try:
                            ...
                            logger.warning(
                                f"[VersatileInterruptRail] ui_notice write_stream 失败：{exc!r}"
                            )
                else:
                    logger.warning(
        ...
        # 持久化 history_info 到 session.state
        # 确保跨 Skill传递不断裂
        if isinstance(normalized, dict):
            has_history_info = "history_info" in normalized
            if has_history_info:
                ctx.session.update_state({
        ...
        # 核心约束：result_message 只走南向通道（tool_result → LLM），
        # 严禁写入 session["response_template"]（北向通道由 response_template_keys 独立控制）
        # ⚠️ 重要：此代码块必须位于 _versatile_cascade_just_resumed 标志设置和 history_info 持久化之后
        # ═══════════════════════════════════════════════════════════════════════════
        result_key = ""
        result_message = ""
        scenario = "default"
        ...
        if isinstance(normalized, dict):
            ...
            normalized.pop("history_info", None)  # 确保存入通道的数据纯净
            if result_key:
                logger.debug(f"[VersatileInterruptRail] 路由标签提取：result_key={result_key!r}")
            if result_message:
                if len(result_message) > 200:
                    logger.warning(
                        f"[VersatileInterruptRail] result_message 过长："
                        f"length={len(result_message)}, 截断预览={result_message[:80]!r}..."
                    )
                logger.debug(f"[VersatileInterruptRail] 路由标签提取：result_message={result_message[:80]!r}")

        # ── 场景 A：后台流转 + 模型感知（南向） ──────────────────────────────────
        if result_key and result_message:
            scenario = "A"
            self._route_to_channel(ctx, result_key, normalized)
            logger.info(
                f"result_key={result_key!r}, result_message={result_message[:50]!r}..., "
                f"scenario={scenario}"
            )
            return self.reject(tool_result={
        ...
        # ── 场景 B：仅模型感知（南向），不缓存 ──────────────────────────────────
        if not result_key and result_message:
            scenario = "B"
            logger.info(
                f"result_message={result_message[:50]!r}..., scenario={scenario}"
            )
            return self.reject(tool_result={
                "status": status or "success",
                "message": result_message,
                "hint": "此步骤仅模型感知，无后台数据存储",
            })

        # ── 默认行为：兼容现有逻辑（保持原有 logger.info + return 不变）─────────
    ...
    async def _sandbox_normalize(
        ...
        if not validate_script_command(command, tag="query_response_analysis_scripts"):
            logger.error(
                "[VersatileInterruptRail] query_response_analysis_scripts "
                "未通过安全校验，拒绝执行沙箱命令：%r",
                command,
            )
            return None, fallback
    ...
    def _route_to_channel(ctx, result_key: str, data: dict) -> None:
        """将数据路由到 tool_data_channel。

        Args:
            ctx: Rail 上下文。
            result_key: 数据在通道中的唯一键名。
            data: 要存储的业务数据字典。

        Raises:
            无。防御性检查已下沉到 ToolDataChannel.store()，此处不再重复。
        """
        channel = ToolDataChannel(ctx.session)
