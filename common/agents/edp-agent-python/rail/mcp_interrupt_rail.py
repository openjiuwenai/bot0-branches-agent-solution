...
通用化设计：
  ...
  - env_vars：LLM 可通过 script_params 传入需要的环境变量 key 列表；若 LLM 未传，
    Rail 使用 _DEFAULT_ENV_VAR_KEYS 自动注入默认 env_vars
...
数据流：
  resolve_interrupt()
    ...
    → 注入持久化字段（history_info / history_params 从 session.state 读取，覆盖 LLM 传入值）
    → 从 script_params 中提取 env_vars key 列表（为空时使用默认 key 列表）→ 从 os.environ 读取值 → 构造环境变量
    → sys_op.shell().execute_cmd() 执行 script_command
    → 解析结果 → session.update_state({
        "mcp_products_data": <parsed_json>,
        "history_info": <从结果提取>,
        "history_params": <从结果提取>,
      })
...
from ._command_security import validate_script_command as _validate_script_command
...
_DEFAULT_ENV_VAR_KEYS: List[str] = [
    "MCP_MASTER_URL",
    "MCP_STANDBY_URL",
    "MCP_ACCESS_TOKEN",
    "MCP_APP_NAME",
...
class MCPInterruptRail(BaseInterruptRail):
    ...
    async def resolve_interrupt(
        ...
        # 安全校验：拒绝含 shell 元字符或不符合白名单格式的 script_command，防止命令注入
        if not _validate_script_command(script_command):
            error_result = self._build_error_result(
                {}, "script_command failed security validation (command injection suspected)"
            )
            ctx.session.update_state({"mcp_products_data": error_result})
            return self.reject(tool_result=error_result)
        ...
        if mcp_result is not None:
            ...
            if isinstance(mcp_result, dict):
                ...
                if mcp_result.get("versatile_query"):
                    mcp_to_versatile_information = mcp_result["versatile_query"]
                    # 清空 versatile_query 字段，避免重复缓存到大模型提示词
                    mcp_result["versatile_query"] = ""
                ...
                # ── 持久化 history_info / history_params 到 session.state ──
                ctx.session.update_state({
                    "history_info": mcp_result.get("history_info", []),
                    "history_params": mcp_result.get("history_params", {}),
                })
                logger.info(
                    f"[MCPInterruptRail] persisted to session: "
                    f"history_info={mcp_result.get('history_info', [])}, "
                    f"history_params={mcp_result.get('history_params', {})}"
                )
            ...
            logger.warning("[MCPInterruptRail] 沙箱执行失败，清空持久化状态并写入空结果到 session state")
    ...
    def _build_skill_input(script_params: dict, ctx) -> dict:
        """合并业务参数和 Rail 自动读取的 mcp_required_params 及持久化状态字段。

        mcp_required_params 从 session.state["original_body"] 读取，
        作为 string 直接注入 SKILL_INPUT，不经过 LLM。

        history_info / history_params 从 session.state 读取，
        覆盖 LLM 可能传入的同名字段，避免 LLM 搬运导致的截断或遗漏风险。
        """
        ...
        # 持久化字段从 session.state 注入，覆盖 LLM 可能传入的值
        base["history_info"] = ctx.session.get_state("history_info") or []
        base["history_params"] = ctx.session.get_state("history_params") or {}
        logger.info(
            f"[MCPInterruptRail] injected from session: "
            f"history_info={base['history_info']}, "
            f"history_params={base['history_params']}"
        )
    ...
    def _build_sandbox_env(skill_input: dict, env_var_keys: List[str]) -> Dict[str, str]:
        """构造沙箱环境变量：SKILL_INPUT 包含 env_vars。

        当 LLM 未传入 env_vars key 列表时，使用 _DEFAULT_ENV_VAR_KEYS 自动注入，
        实现敏感配置（URL、Token 等）不经过 LLM 的安全传递。
        """
        if not env_var_keys:
            env_var_keys = list(_DEFAULT_ENV_VAR_KEYS)
            logger.info(f"[MCPInterruptRail] env_var_keys 为空，使用默认 key 列表：{env_var_keys}")
    ...
    def _build_error_result(skill_input: dict, reason: str) -> dict:
        ...
        MCP 调用失败时清空 history_info 和 history_params，
        使下次调用不继承上次条件。
        """
        return {
            "products": [],
    ...
    async def _execute_sandbox(self, script_command: str, sandbox_env: Dict[str, str]) -> Optional[dict]:
        """在沙箱中执行脚本，返回 JSON 结果。"""
        # 防御性校验（defense in depth）：即便上层已校验，此处再次拦截
        if not _validate_script_command(script_command):
            logger.error(
                "[MCPInterruptRail] _execute_sandbox 收到非法 script_command，拒绝执行：%r",
                script_command,
            )
            return None
