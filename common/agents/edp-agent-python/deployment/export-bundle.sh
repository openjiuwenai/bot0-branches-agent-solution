...
#   ├── edpagent.<日期>.tar    docker save 出的镜像（离线 load 用）
#   ├── config/
#   │   ├── a2a_service.env
...
# ── 2. 导出镜像 ─────────────────────────────────────────────────────
echo "[1/3] docker save → edpagent.${STAMP}.tar"
docker save "$IMAGE_TAG" -o "$STAGE_DIR/edpagent.${STAMP}.tar"
echo "     $(du -h "$STAGE_DIR/edpagent.${STAMP}.tar" | cut -f1)"
...
echo ""
echo "将 $FINAL_PATH 传输到客户机器，解压后执行 ./import-bundle.sh 镜像包名"
