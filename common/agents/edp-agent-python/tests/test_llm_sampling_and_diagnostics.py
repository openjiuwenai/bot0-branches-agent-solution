...
def _mk_rail_ctx(tool_name: str, tool_args):
    """合成 AgentCallbackContext 给 LogRail.before_tool_call 用。"""
...
def _mk_rail():
    ...
    return LogRail(model_name="test-model", tools=[])


async def test_rail_logs_tool_keyword_for_business_tool(captured_logs):
    """call_versatile 必须打 [EDP-LLM-TOOL]。"""
    rail = _mk_rail()
    ...
    blob = " ".join(matched)
    assert "call_versatile" in blob
    assert "purchase_product" in blob, "args 全文未落盘"


async def test_rail_logs_tool_keyword_for_suppressed_ask_user(captured_logs):
    """ask_user 也必须打 [EDP-LLM-TOOL]（LogRail 不区分 suppress）。"""
    rail = _mk_rail()
    args = {
    ...
    matched = [line for line in captured_logs if "[EDP-LLM-TOOL]" in line]
    assert matched, "ask_user 也必须有 [EDP-LLM-TOOL]"
    blob = " ".join(matched)
...
async def test_rail_tool_log_handles_string_args(captured_logs):
    ...
    matched = [line for line in captured_logs if "[EDP-LLM-TOOL]" in line]
    assert matched
    assert "ask_user" in " ".join(matched)
