...
def _run_case(env: dict, case_name: str, case: dict, runs: int,
    ...
    for i in range(1, runs + 1):
        ...
        if "tool_name" in info:
            ...
            logger.info(f"        └─ tool={info['tool_name']!r} args={args_short!r}")
        if "exception" in info:
            logger.info(f"        └─ exception: {info['exception']}")
        if "dump" in info:
            logger.warning(f"        └─ ⚠ raw response dumped: {info['dump']}")
            logger.info(f"        └─ message keys: {info.get('message_keys')}")
...
def main() -> None:
    ...
    logger.info(f"📍 headers（敏感字段脱敏后）= {safe_headers}")
    if env["extra_headers"]:
        logger.info(f"📍 extra_headers 注入 {len(env['extra_headers'])} 个字段")
    ...
    if total_empty > 0:
        logger.info(f"      详细 dump 在 {dump_dir}/，把目录里的文件贴出来即可定位")
    logger.info('=' * 78)
