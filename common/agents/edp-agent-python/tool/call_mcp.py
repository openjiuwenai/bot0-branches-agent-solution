        ...
        input_params={
            ...
            "properties": {
                "script_command": {
                    "type": "string",
                    "description": (
                        "要执行的 Python 脚本命令，脚本在 skills/ 目录下执行。"
                        "示例：'python interact_finance_rec_skill/scripts/run_mcp_recommend.py'。"
                    ),
                },
                "script_params": {
                    "type": "string",
                    "description": (
                        ...
                        "必须包含：mcp_params（筛选条件字典）和 empty_result_template_key（固定值 \"mcp_result_empty\"）。"
                        "示例：'{\"mcp_params\": {\"filterRiskLevel\": \"2\"}, "
                        "\"empty_result_template_key\": \"mcp_result_empty\"}'。"
                        "禁止传入 mcp_required_params、history_info、history_params（由系统自动注入）。"
                    ),
                },
            },
