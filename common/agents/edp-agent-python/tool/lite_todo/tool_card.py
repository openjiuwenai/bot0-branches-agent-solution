...
2. **You MUST**：如果本轮需要调用本工具写入或更新 todo 列表，本轮只调用 lite_todo_write；不要在同一 assistant message / 同一轮工具调用中同时调用业务工具（call_versatile / call_mcp）。收到 lite_todo_write 工具结果后的下一轮，再调用业务工具执行下一步。
3. **You MUST**：每个 step_id 在 todos 列表里只能出现一次；不要重复
4. **You MUST**：todos 列表只放打算做的项——不打算做的步骤**不要**放进列表
5. **You MUST**：进入"业务步骤目录"中某一项前，再次调用本工具把对应项 status 仍然标 `pending`（"正在执行哪一项"由框架的 todo_start/todo_status 事件单独表达，**不要**自己加 in_progress）；翻 `done` 必须对应该 step 绑定 skill 的一次实际成功执行（典型表现为 call_versatile / call_mcp / call_multiagent `tool_end success=true` 或 `status=success`）。`call_multiagent` 返回成功后，**必须再次调用本工具**将完成的 step 翻 `done`，**严禁**跳过状态刷新直接进入下一步或输出 final_answer。`ask_user` 收到用户回包**不构成**任何 step 的完成依据；用户回包后下一步必须是调用业务工具，而不是更新 todo
