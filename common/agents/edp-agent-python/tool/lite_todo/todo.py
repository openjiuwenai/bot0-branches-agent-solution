...
class LiteTodoWriteTool(Tool):
    ...
    async def invoke(self, inputs: Input, **kwargs) -> _ToolOutput:
        ...
        # 空列表 → 清 state，不返回任何 todos（不发事件，避免空清单污染前端）
        # All-done → 清 state（state 里不保留全完成快照），但仍返回完整 todos
        #           让 _parse_lite_todo_tool_result 能派发"全部完成"的 todolist_* 事件给前端
        all_done = bool(todos) and all(t.status.value == "done" for t in todos)
        if not todos:
            await self._manager.clear(session)
            persisted = []
        else:
            # all_done 也走 clear 清掉 state（LiteTodoManager 未提供单独的"清但返回"接口，
            # 这里复用 clear 清掉 state，但 persisted 仍填充完整 todos 返回给前端）
            if all_done:
                await self._manager.clear(session)
            else:
