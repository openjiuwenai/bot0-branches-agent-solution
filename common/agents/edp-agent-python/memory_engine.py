...
_memory_engine = None
# Underlying connection resources tracked for graceful shutdown.
# They are created during init and released by close_memory_engine().
...
async def _close_resource(resource: Any, close_method: str, name: str) -> None:
    """Close a single resource, isolating any error.

    A failing close must not prevent the remaining resources from being
    released, so exceptions -- including ``asyncio.CancelledError`` (which on
    Python 3.9+ inherits from ``BaseException``, not ``Exception``) -- are
    swallowed and reported as warnings.

    Both sync and async close methods are supported: if calling the method
    returns an awaitable it is awaited, otherwise the synchronous call is
    considered sufficient. This avoids spurious ``TypeError`` warnings when a
    close method happens to be synchronous (e.g. an older SQLAlchemy engine
    whose ``dispose()`` is not a coroutine).
    """
    if resource is None:
        return
    try:
        ...
        if inspect.isawaitable(result):
            await result
        logger.debug("[DPA][MEMORY] Closed {} successfully", name)
    except (Exception, asyncio.CancelledError) as exc:  # noqa: BLE001
        logger.warning("[DPA][MEMORY] Failed to close {}: {}", name, exc)


async def _close_all_resources() -> None:
    """Close every underlying connection resource tracked by this module.

    Resources are released in reverse order of creation. Each close is
    independent, so one failure cannot block the others. Module-level
    references are reset afterwards so the engine can be re-initialized.
    """
    global _redis_client, _es_client, _db_engine
    await _close_resource(_es_client, "close", "elasticsearch client")
    await _close_resource(_db_engine, "dispose", "db engine")
    await _close_resource(_redis_client, "aclose", "redis client")
    _es_client = None
    _db_engine = None
    _redis_client = None


async def close_memory_engine() -> None:
    """Gracefully shut down the memory engine and release all connections.

    Safe to call when the engine was never initialized or already closed.
    After this returns, :func:`get_memory_engine` will raise until
    ...
    # Drop the engine reference first so concurrent callers stop using it
    # while the underlying connections are being torn down.
    _memory_engine = None
    await _close_all_resources()
    logger.info("[DPA][MEMORY] Memory engine closed")
...
async def init_memory_engine(settings: DPASettings) -> None:
    ...
    global _memory_engine, _redis_client, _es_client, _db_engine
    ...
    # Everything below either opens a connection or depends on one, so it is
    # wrapped in a single try/except: on any failure the already-created
    # connections are released via _close_all_resources() before re-raising.
    try:
        ...
        logger.exception("[DPA][MEMORY] Memory engine initialization failed, cleaning up created connections")
        await _close_all_resources()
        raise
