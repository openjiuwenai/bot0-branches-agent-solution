...
└── scenarios/                                 # 场景目录（来自 agent-store）
    ...
    └── abc-loan-scenario/                         # 示例贷款场景场景（精简示例）
        └── governance/
            ├── actrule.yaml                   #   2 步 DAG（贷款申请 → 审批）
            └── planrule.yaml                  #   示例贷款场景角色 + scope
```
...
### abc-loan-scenario（示例贷款场景）— 精简场景

最小可用示例，仅含 governance 层：
- **2 步 DAG**：loan_apply → loan_approval
- **scope 约束**：允许示例贷款场景贷款，禁止理财/信用卡

切换方式：

```bash
java -jar target/customer-agent-app-1.0.0.jar \
  --deep-agent.scenario-home=./scenarios/abc-loan-scenario
```
...
**Q: 如何切换场景？**
A: 修改 `application.yml` 的 `scenario-home`，或通过命令行参数 `--deep-agent.scenario-home=./scenarios/abc-loan-scenario` 覆盖。
