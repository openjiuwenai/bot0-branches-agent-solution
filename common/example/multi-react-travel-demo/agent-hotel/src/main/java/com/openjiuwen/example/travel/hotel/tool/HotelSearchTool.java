...
package com.openjiuwen.example.travel.hotel.tool;

import com.openjiuwen.core.foundation.tool.ToolCard;
import com.openjiuwen.core.foundation.tool.function.LocalFunction;
import com.openjiuwen.example.travel.hotel.mock.Hotel;
...
public final class HotelSearchTool extends LocalFunction {
    ...
    static Map<String, Object> execute(MockHotelInventory inventory, Map<String, Object> inputs) {
        String cityName = asString(inputs.get("cityName")).orElse(null);
        ...
        OptionalDouble maxPrice = asDouble(inputs.get("maxPricePerNight"));
        OptionalInt minStar = asInt(inputs.get("minStar"));
        List<String> brandWhitelist = asStringList(inputs.get("brandWhitelist"));
        List<String> keywords = parseKeywords(asString(inputs.get("keyword")).orElse(null));
        int pageNum = Math.max(1, asIntOrDefault(inputs.get("pageNum"), 1));

        List<Hotel> filtered = new ArrayList<>();
        for (Hotel h : cityHotels) {
            if (maxPrice.isPresent() && h.lowestPrice() > maxPrice.getAsDouble()) {
                continue;
            }
            if (minStar.isPresent() && h.star() < minStar.getAsInt()) {
                continue;
            }
    ...
    private static Map<String, Object> toBrief(
            Hotel h, OptionalDouble maxPrice, OptionalInt minStar, List<String> brandWhitelist) {
    ...
    private static OptionalDouble asDouble(Object v) {
        if (v == null) {
    ...
    private static OptionalInt asInt(Object v) {
        if (v == null) {
    ...
    private static int asIntOrDefault(Object v, int fallback) {
        return asInt(v).orElse(fallback);
    }
     ...
     *
     * @param inventory the hotel inventory to query
     * @param inputs    the raw tool input map (city + dates + travel-standard filters)
     * @return the tool result map (≤6 candidate hotels, or an error payload)
     */
