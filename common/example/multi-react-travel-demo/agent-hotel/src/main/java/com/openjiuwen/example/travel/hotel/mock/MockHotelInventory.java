...
package com.openjiuwen.example.travel.hotel.mock;
...
public final class MockHotelInventory {
     ...
     *
     * @param classpathResource the classpath resource path of the inventory JSON file
     */
     ...
     *
     * @return the total hotel count across every city
     */
     ...
     *
     * @return the number of distinct cities in the inventory
     */
     ...
     *
     * @param cityName the city name in any spelling the LLM may produce
     * @return the hotels in that city, or an empty list if the city is unknown
     */
     ...
     *
     * @param hotelId the hotel id to look up
     * @return the matching hotel, or {@link Optional#empty()} if no hotel has that id
     */
    public Optional<Hotel> findById(String hotelId) {
        return hotelId == null ? Optional.empty() : Optional.ofNullable(hotelsById.get(hotelId));
    }
     ...
     *
     * @param raw the city name as written by the LLM
     * @return the canonical storage key, or {@code ""} when {@code raw} is null
     */
