...
- Call Tree：SDK `InvocationCall.callTree()` 的实时快照。
- SDK Events：标准 `InvocationEvent` 时间线。
- Final Snapshot：`InvocationSnapshot`，包括恢复线索和树完整度。
- Runtime Wire：Mock Runtime 实际收到的 A2A JSON-RPC 请求及 `Last-Event-ID`。
...
`BLOCKING/ASYNC` 可与任意普通场景组合。按当前协议，它们的调用树应为
`UNAVAILABLE_FOR_MODE` 或 `PARTIAL`，不能当作完整过程树。
