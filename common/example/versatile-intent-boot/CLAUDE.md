...
- `scripts/cli_llm_intent.py` — 纯标准库 Python CLI 客户端（参考 a2a-samples `helloworld/test_client.py`）。`start` 子命令读 `.env`（`LLM_API_KEY` 等，见 `.env.example`，`.env` 已 gitignore）一键拉起 6 进程栈并退出时清理；`card`/`scenario a|b|c|all`/`chat` 假定栈已运行，经 `POST /v1/query` 驱动 L1（请求体与 shell 的 `send_q` 一致，`--stream` 走 SSE）。
