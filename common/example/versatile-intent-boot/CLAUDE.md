...
mvn -Dtest=ReclassifyServeOrchestratorTest test   # run a single test class
```
...
| `downstream` | 8083 | Terminal business node (no `intents`/`intent-agent-mapping`; can trigger reclassify) |
...
- `routecache/` — Multi-turn route cache (L1 only). Caches the next-hop `agent_id` per `conversationId`; subsequent turns skip L1 by synthesizing an `a2a_delegate` interrupt. In-process `ConcurrentHashMap` + TTL, no Redis. Invalidated on TTL, `clearSession`, or reclassify signal. `CachedVersatileAgentHandler` decorates the base handler.
- `reclassify/` — When L2 returns an ambiguous `intent_id`, `ReclassifyServeOrchestrator` (a `ServeOrchestrator` decorator installed via `ReclassifyOrchestratorPostProcessor`) re-runs L1 with the L2 answer appended as an assistant message, up to `max-reclassify` times. Invalidates the L1 route cache before each retry. `max-reclassify=0` fails fast with `VERSATILE_INTENT_RECLASSIFY_LIMIT`.
...
- `intentllm/` — LLM 意图分类 `AgentHandler` 变体（classify 模式），`intent-llm.enabled=true` 时抢 `AgentHandler` 槽位（与 Versatile 二选一，即意图对接 SPI）。`LlmIntentAutoConfiguration` 在 route-cache bean 存在时用 `CachedVersatileAgentHandler` 包裹，故 L1 可同时启用 route-cache。imports 中置于 RouteCacheAutoConfiguration 之后。
- `mock/` — `MockVersatileController` (canned Versatile SSE keyed by `(agentId, query)`) and `MockA2AGatewayController` (forwarding proxy for tests).

### Configuration prefix

Runtime config lives under `openjiuwen.service` (versatile, a2a-gateway, card-resolver). This module's own features use `openjiuwen.service.versatile.intent-reclassify`, `openjiuwen.service.versatile.route-cache`, and `openjiuwen.example.direct-chain`. See `README.md` for the full YAML reference.
