...
class CriteriaReplanBridgeRailTest {
    ...
    // ==================== Exit 1: verify pass → forceFinish(verified) ====================

    @Test
    void verifyPass_locksVerifiedTerminal() {
        // Given: RuleBasedCriteriaVerifier matches keywords in output
        ReplanRail replanRail = new ReplanRail(3);
        CriteriaReplanBridgeRail rail = new CriteriaReplanBridgeRail(
        ...
        assertThat(ctx.hasForceFinishRequest())
                .as("verify pass must fire requestForceFinish(verified=true)")
                .isTrue();
        // mutation-RED: strip forceFinish(verifiedResult) → hasForceFinishRequest false → RED
    }

    @Test
    void verifyPass_resultContainsVerifiedTrue() {
        ReplanRail replanRail = new ReplanRail(3);
        CriteriaReplanBridgeRail rail = new CriteriaReplanBridgeRail(
                new RuleBasedCriteriaVerifier(),
                List.of("建议", "债券"),
    ...
    // ==================== Exit 2: verify fail + under limit → pushSteering, no forceFinish ====================

    @Test
    void verifyFail_underLimit_pushSteering_noForceFinish() {
        ...
        assertThat(ctx.hasForceFinishRequest())
                .as("under-limit verify fail must NOT forceFinish — loop continues for retry")
                .isFalse();
    ...
    @Test
    void verifyFail_underLimit_replanCountIncremented() {
        ReplanRail replanRail = new ReplanRail(3);
        CaptureSteeringQueue steeringQ = new CaptureSteeringQueue();
        CriteriaReplanBridgeRail rail = new CriteriaReplanBridgeRail(
                new RuleBasedCriteriaVerifier(),
        ...
        assertThat(replanRail.replanCount())
                .as("verify-fail retry must increment shared replan count")
                .isEqualTo(beforeCount + 1);
    }

    // ==================== Exit 3: verify fail + over limit → forceFinish(degraded) ====================

    @Test
    void verifyFail_overLimit_forceFinishDegraded() {
        ...
        assertThat(ctx1.hasForceFinishRequest())
                .as("first fail (under limit) must NOT forceFinish")
                .isFalse();
    ...
    @Test
    void verifyFail_overLimit_resultContainsDegradedTrue() {
        ...
        assertThat(cap.result())
                .containsKey(CriteriaReplanBridgeRail.RETRY_COUNT_KEY);
    }
    ...
    // ==================== GradientVerifier integration — gradient steering hints ====================

    @Test
    void gradientVerifier_partialCoverage_steeringContainsGradientHint() {
        ReplanRail replanRail = new ReplanRail(3);
        CaptureSteeringQueue steeringQ = new CaptureSteeringQueue();
        CriteriaReplanBridgeRail rail = new CriteriaReplanBridgeRail(
                new GradientVerifier(),
                List.of("需要包含GDP、CPI和通胀率分析"),
                replanRail);

        // Output covers GDP but not CPI/通胀率 → partial coverage
        AgentCallbackContext ctx = ctxWithFinalAnswer("GDP增长5%", steeringQ);
        rail.afterModelCall(ctx);

        assertThat(steeringQ.captured)
                .as("gradient verifier partial coverage must push gradient hint")
                .isNotEmpty();
        String hint = steeringQ.captured.get(0);
        assertThat(hint)
                .as("gradient hint must mention covered dimensions")
                .contains("已覆盖")
                .contains("需要包含gdp");
        assertThat(hint)
                .as("gradient hint must mention missing dimensions")
                .contains("请补充")
                .contains("cpi和通胀率分析");
        // mutation-RED: strip gradient branch in buildCorrectionHint (always use
        // standard fallback) → hint does not contain "已覆盖" → assertion fails → RED
    }

    @Test
    void gradientVerifier_belowFloorCoverage_steeringListsAllDimensions() {
        ReplanRail replanRail = new ReplanRail(3);
        CaptureSteeringQueue steeringQ = new CaptureSteeringQueue();
        CriteriaReplanBridgeRail rail = new CriteriaReplanBridgeRail(
                new GradientVerifier(),
        ...
        // Output covers only GDP → 1/5=0.2 < floor 0.25 → coverage-floor branch
        AgentCallbackContext ctx = ctxWithFinalAnswer("GDP增长5%", steeringQ);
        rail.afterModelCall(ctx);

        assertThat(steeringQ.captured)
                .as("below-floor coverage must push steering hint")
                .isNotEmpty();
        String hint = steeringQ.captured.get(0);
        assertThat(hint)
                .as("below-floor hint must list all required dimensions")
                .contains("需要覆盖的维度")
                .contains("gdp")
                .contains("cpi")
                .contains("通胀率")
                ...
                .contains("财政政策");
        assertThat(hint)
                .as("below-floor hint must NOT claim partial coverage")
                .doesNotContain("已覆盖")
                .doesNotContain("请补充");
        // mutation-RED: remove gradient branch → hint falls back to standard format
        // → does not contain "需要覆盖的维度" → RED
    }

    @Test
    void gradientVerifier_fullCoverage_noSteering_forceFinishVerified() {
        ReplanRail replanRail = new ReplanRail(3);
        CaptureSteeringQueue steeringQ = new CaptureSteeringQueue();
        CriteriaReplanBridgeRail rail = new CriteriaReplanBridgeRail(
                new GradientVerifier(),
        ...
        AgentCallbackContext ctx = ctxWithFinalAnswer(
                "GDP增长5%，CPI温和上涨，通胀率2%", steeringQ);
        rail.afterModelCall(ctx);

        assertThat(ctx.hasForceFinishRequest())
                .as("full coverage must forceFinish(verified)")
                .isTrue();
        assertThat(steeringQ.captured)
                .as("full coverage must NOT push steering")
                .isEmpty();
        // mutation-RED: strip dimension-level coverage check (always fail) →
        // pushSteering called instead of forceFinish → RED
    }
