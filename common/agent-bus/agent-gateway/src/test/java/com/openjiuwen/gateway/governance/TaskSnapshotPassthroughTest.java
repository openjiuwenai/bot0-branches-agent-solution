...
class TaskSnapshotPassthroughTest {
    ...
    void setUp() {
        ...
        router = new Router(rdc, runtime, sticky, new com.openjiuwen.gateway.routing.DefaultAgentResolver("default-agent"));
    }
    ...
    void getTaskSnapshotPassedThroughWithAllA2aFields() throws Exception {
        ...
        String snapshot = "{\"jsonrpc\":\"2.0\",\"id\":\"req-1\",\"result\":{"
                ...
                + "\"metadata\":{\"customKey\":\"customVal\",\"agentEvent\":{\"source\":\"agent-b1\",\"batchId\":\"b1\"}}"
                + "}}";
        ...
        assertThat(root.path("result").path("metadata").path("agentEvent").path("source").asText()).isEqualTo("agent-b1");
    }
