 ...
 *   <li><b>软观察</b>：模型实例的 getInjectionMode() 在 beforeModelCall 后
 ...
 * <ul>
 *   <li>真 LLM e2e 验证实例状态通道通信（rail → SystemPromptInjectingModel）在全链路工作</li>
 *   <li>Plan-related specific config is tested via mock tests (PreCompletionChecklistRailTest)</li>
 *   <li>本 e2e 验证 "整个链通"——rail.beforeModelCall → 实例状态 → EnforcingModel.invoke</li>
 * </ul>
 *
 * <p>已知局限（caveats）：
 * <ul>
...
class PreCompletionChecklistRailE2eTest {
    ...
    void planPhaseInjectsPlanMode() {
        ...
        var cliCfg = ModelClientConfig.builder().clientId("checklist-e2e-" + System.nanoTime()).clientProvider("OpenAI").timeout(180_000)
        ...
        PreCompletionChecklistRail rail = new PreCompletionChecklistRail(2, model.injectionState());
        agent.registerRail(rail);

        // Before any call: the model-owned channel should be NONE
        assertThat(model.getInjectionMode()).as("model mode must start as NONE").isEqualTo(InjectionMode.NONE);
        ...
        assertThat(model.getInjectionMode())
    ...
    void afterPlanRoundsSwitchesToBuildMode() {
        ...
        PreCompletionChecklistRail rail = new PreCompletionChecklistRail(1, model.injectionState());
        agent.registerRail(rail);
     ...
     *
     * <p>软观察：通过目标模型的 peekPhaseOverride() 检查。
    ...
    void stagnationDetectionInjectsBreakLoop() {
        ...
        // → sets the target model's phase override
