...
class AgentRegistryMigrationTest {
    ...
    void agent_registry_mvp_has_four_field_primary_key() {
        ...
        assertThat(pkColumns)
                .as("FEAT-016 V6: PK must be (tenant_id, agent_id, service_id, instance_id)")
    ...
    // ---- V8: source revision + deployment_service_id -------------------

    @Test
    void v8_registry_source_state_table_exists() {
        Integer count = jdbc.queryForObject(
                "SELECT COUNT(*) FROM information_schema.tables "
                + "WHERE table_schema = 'public' AND table_name = 'registry_source_state'",
                Integer.class);
        assertThat(count).isEqualTo(1);
    }

    @Test
    void v8_deployment_service_id_column_exists() {
        Integer count = jdbc.queryForObject(
                "SELECT COUNT(*) FROM information_schema.columns "
                + "WHERE table_schema = 'public' AND table_name = 'agent_registry_mvp' "
                + "AND column_name = 'deployment_service_id'",
                Integer.class);
        assertThat(count).isEqualTo(1);
    }
    ...
    private void insertValidRow(String tenantId, String agentId) {
        ...
        // service_id) PK.
        jdbc.update("INSERT INTO agent_registry_mvp ("
                + "tenant_id, agent_id, service_id, instance_id, source_id, source_revision, "
                + "agent_name, framework_type, "
                + "route_key, contract_version, capability_version, "
                + "endpoint_url, weight, status, lifecycle_status, effective_health, "
                + "freshness, lease_expires_at, last_validated_at) "
                + "VALUES (?, ?, ?, ?, 'legacy-push', 0, 'name', 'JIUWEN', "
                + "'rk', '1.0', '1.0', 'http://x', 100, 'ONLINE', "
                + "'ACTIVE', 'HEALTHY', 'FRESH', CURRENT_TIMESTAMP + INTERVAL '1 hour', "
                + "CURRENT_TIMESTAMP)",
                tenantId, agentId, "svc-" + agentId, "svc-" + agentId);
    }
}
