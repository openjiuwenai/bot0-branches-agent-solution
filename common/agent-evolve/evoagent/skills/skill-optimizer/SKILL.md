...
description: 给定一个服务器侧种子数据集路径 + 目标 agent 名 + 待优化 skill 名，经 adapter 读取 baseline、调用优化器服务完成"提交优化→轮询→报告前后对比→经 adapter 回写 skill→调 agent 验证"全流程。当用户说"用这批轨迹/这个数据集优化 X agent 的 Y skill""改进 abc_agent/edp_agent 的 skill""跑一轮 skill 优化"时使用。
---
...
| 参数 | 必填 | 说明 |
|---|---|---|
| `agent_name` | 是 | 目标 agent 名（如 abc_agent、edp_agent） |
