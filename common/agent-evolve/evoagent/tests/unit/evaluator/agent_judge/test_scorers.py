...
class TestWeightedSumScorer:
    ...
    def test_missing_weight_treated_as_zero_and_renormalized(self) -> None:
        # b has no weight entry → weight 0; renormalize over a only
        out = WeightedSumScorer().score([_j("a", 0.5), _j("b", 1.0)], weights={"a": 1.0})
        assert out == 0.5

    def test_no_positive_weight_falls_back_to_mean(self) -> None:
        out = WeightedSumScorer().score([_j("a", 0.4), _j("b", 0.8)], weights={})
        assert out == pytest.approx(0.6)

    def test_empty_judgments_zero(self) -> None:
        assert WeightedSumScorer().score([], weights={"a": 1.0}) == 0.0
...
class TestTaskCompletionGatedScorer:
    def test_tc_zero_gates_to_zero(self) -> None:
    ...
    def test_tc_times_weighted_others(self) -> None:
    ...
    def test_tc_absent_falls_back_to_weighted_sum(self) -> None:
        out = TaskCompletionGatedScorer().score(
    ...
    def test_only_tc_present_returns_tc(self) -> None:
        out = TaskCompletionGatedScorer().score(
            [_j("task_completion", 0.7)], weights={"task_completion": 1.0}
        )
        assert out == 0.7

    def test_tc_own_weight_not_applied_to_others(self) -> None:
...
class TestScorerRegistry:
    ...
    def test_get_returns_scorer(self) -> None:
        s = get_scorer("task_completion_gated")
        assert isinstance(s, WeightScorer)

    def test_default_scorer_types(self) -> None:
