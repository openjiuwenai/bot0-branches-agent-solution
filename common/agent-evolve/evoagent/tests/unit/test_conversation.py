...
def test_counter_increments() -> None:
    ...
    assert id1 == "run1:train:1:case-001"
    assert id2 == "run1:train:2:case-002"
    assert id3 == "run1:val:3:case-001"  # counter 跨 phase 递增
...
def test_different_run_ids_produce_different_ids() -> None:
    ...
    assert id1 != id2
    assert id1.startswith("run1:")
    assert id2.startswith("run2:")
