...
span dict 形状见 kafka_consumer.otlp_parser.parse_span:
    ...
    service_name/conversation_id/attributes(dict)/resource_attributes(dict)/...

汇总规则 (设计文档 §3 traces 表):
    - 根 span = kind=SERVER 且 parent_span_id 为空; root_span_id/user_id/
      request_summary/response_summary 取自根 span attributes。
    - start_time = min(各 span start_time); end_time = max(各 span end_time)。
    - span_count = len(spans); status = 最差状态 (ERROR > OK > UNSET)。
    - conversation_id/openjiuwen_trace_id/service_name: 根 span 优先, 否则取首个非空。
"""
...
def compute_trace_summary(trace_id: str, spans: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "trace_id": trace_id,
        "conversation_id": (root or first).get("conversation_id") or _first_attr("session.id"),
        "user_id": a_root.get("user.id"),
        ...
        "response_summary": a_root.get("openjiuwen.http.response_summary"),
        "openjiuwen_trace_id": _first_attr("openjiuwen.trace.id"),
    }
