/**
 * DFW-RAG - Embedding Config Page Script
 * 使用模板动态渲染表单，支持新增、编辑、删除、测试、设为激活。
 */
...
(function () {
  ...
  async function loadTemplates() {
    try {
      const data = await API.request('GET', 'api/embedding/templates');
  ...
  async function loadProfiles() {
    ...
    try {
      const profiles = await API.request('GET', 'api/embedding/profiles');
  ...
  function renderProfileList(profiles) {
    ...
    if (!profiles.length) {
      container.innerHTML = '<div class="empty-cell">暂无 Embedding 配置，点击右上角"新增配置"添加。</div>';
      return;
    }
    ...
    profiles.forEach(p => {
      ...
      const activeBadge = p.active ? '<span class="badge badge-success" style="margin-left: 6px;">当前激活</span>' : '';
      tr.innerHTML = `
        <td><strong>${App.escapeHtml(p.name)}</strong>${activeBadge}</td>
        <td><span class="badge badge-info">${App.escapeHtml(p.mode || p.template)}</span></td>
        ...
        <td class="text-end">
          <div class="action-buttons">
            ${p.active ? '' : `<button type="button" class="btn btn-warning btn-sm btn-activate" data-id="${App.escapeHtml(p.id)}" title="设为激活">${ICONS.star} 激活</button>`}
  ...
  async function saveTemplate() {
    ...
    try {
      const result = await API.request('POST', 'api/embedding/templates', { yaml });
      if (result.success) {
  ...
  async function viewTemplate(name) {
    ...
    try {
      const data = await API.request('GET', `api/embedding/templates/${encodeURIComponent(name)}/yaml`);
  ...
  function openAddModal() {
    ...
    document.getElementById('modalTitle').textContent = '新增 Embedding 配置';
  ...
  async function openEditModal(id) {
    ...
    try {
      const profile = await API.request('GET', `api/embedding/profiles/${id}`);
      ...
      document.getElementById('modalTitle').textContent = '编辑 Embedding 配置';
      hideTestResult();
  ...
  async function onTemplateChange() {
    ...
    App.showLoading('加载模板…');
    try {
      const scaffold = await API.request('GET', `api/embedding/templates/${templateName}/scaffold`);
  ...
  async function renderFormFromTemplate(templateName, values) {
    const data = await API.request('GET', `api/embedding/templates/${templateName}/form`);
  ...
  async function saveProfile() {
    ...
    try {
      ...
      await API.request('POST', 'api/embedding/profiles', data);
  ...
  async function testDraft() {
    ...
    try {
      const data = collectProfileData();
      const result = await API.request('POST', 'api/embedding/profiles/test-draft', data);
  ...
  async function testSavedProfile(id) {
    App.showLoading('测试中…');
    try {
      const result = await API.request('POST', `api/embedding/profiles/${id}/test`, {});
      showTestResult(result);
    } catch (err) {
      App.showToast('测试失败：' + err.message, 'error');
    } finally {
  ...
  async function activateProfile(id) {
    App.showLoading('切换中…');
    try {
      const result = await API.request('POST', `api/embedding/profiles/${id}/active`, {});
      if (result.success) {
        App.showToast(result.message || '已切换激活配置', 'success');
        await loadProfiles();
      } else {
        App.showToast(result.message || '切换失败', 'error');
      }
    } catch (err) {
  ...
  async function deleteProfile(id) {
    const ok = await App.confirm('确定要删除这个 Embedding 配置吗？');
    if (!ok) return;

    App.showLoading('删除中…');
    try {
      const result = await API.request('DELETE', `api/embedding/profiles/${id}`);
      if (result.success) {
