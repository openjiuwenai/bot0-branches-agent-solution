...
export function UnitTable({ rows, orig, onChange, onAdd, onDelete }: {
  rows: DeviceRow[]
  orig: DeviceRow[]
  ...
  if (!rows.length) return <EmptyHint text="暂无装置数据" />
  return (
    <div className="rounded-xl border border-[#E6EAF1] bg-white shadow-sm overflow-hidden">
      <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-100 bg-slate-50/60">
        <CardHead icon={Cog} title="装置管理" accent="from-slate-600 to-slate-700"
          hint={`共 ${rows.length} 台装置 · 行内编辑后点右上角保存 · 新增行需填写ID`} />
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 text-[12px] text-slate-500 border-b border-slate-100">
              <th className="text-left font-medium px-3 py-2 min-w-[100px]">装置ID</th>
              ...
              <th className="text-left font-medium px-3 py-2 w-44">关联主数据装置</th>
      ...
      <button onClick={onAdd}
        ...
        <Plus className="w-3.5 h-3.5" />新增装置
      </button>
    </div>
  )
}
